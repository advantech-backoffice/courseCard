...
function getAssignmentLink(topic) {
 return typeof topic === "object" ? (topic.assignmentLink || "") : "";
}
...
export default function CourseDetail() {
 ...
 const markTopicCompleted = async (moduleId, topicName) => {
  ...
  try {
   ...
   setUserProgress((prev) => [
    ...prev,
    { topicKey, completedAt: data.completedAt, activityType: data.activityType },
   ]);
  } catch (err) {
   console.error(err);
  }
 ...
 const progress = Math.round(
  (userProgress.length / totalTopics) * 1000
 ) / 10;
 ...
 return (
  ...
  {isCompleted && (
   <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-6 text-center">
    ...
    <h3 className="text-lg font-bold text-emerald-500 mb-1">Course Completed</h3>
    <p className="text-sm text-zinc-400">
     This course has been marked as completed. No further changes can be made.
    </p>
   </div>
  )}
       ...
       <div className="px-8 pb-6 pt-2 border-t border-zinc-800 bg-zinc-900">
        <ul className="space-y-2">
         ...
         {module.module_content?.map((topic, tIndex) => {
          ...
          const assignmentLink = getAssignmentLink(topic);
          const topicKey = `${module.module_name}-${topicName}`;
          const completionData = userProgress.find(t => t.topicKey === topicKey);
          const topicCompleted = !!completionData;

          return (
           <li
            key={tIndex}
            onClick={() => {
             if (!isCompleted && !topicCompleted) {
              markTopicCompleted(module.module_name, topicName);
             }
            }}
            ...
            ${
             topicCompleted
              ...
              : "hover:bg-zinc-800 text-zinc-300 border border-transparent" + (!isCompleted ? " cursor-pointer" : "")
            }`}
           >
            <div className="flex items-center">
             {topicCompleted ? (
            ...
            <div className="flex items-center gap-2">
             {assignmentLink && !topicCompleted && (
              <a
               href={assignmentLink}
               target="_blank"
               rel="noopener noreferrer"
               onClick={(e) => e.stopPropagation()}
               className="flex items-center gap-1 text-xs text-blue-400 hover:text-blue-300 transition px-2 py-1 rounded-lg bg-blue-500/10 border border-blue-500/20"
              >
             ...
             {topicCompleted ? (
              ...
              !assignmentLink && <Clock className="w-4 h-4 text-zinc-500" />
             )}
            </div>
           </li>
          );
