...
function ReportSection({ title, subtitle, icon: Icon, children, onExcel, onPdf }) {
 ...
 className="w-full flex items-center justify-between p-6 hover:bg-zinc-50 :bg-zinc-800/50 transition cursor-pointer"
 >
...
const SELECT_CLASSES = "px-3 py-2 bg-zinc-50 border border-zinc-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-600 outline-none transition";
...
export default function AdminReports() {
 ...
 <div className="h-48 bg-zinc-200 rounded-3xl animate-pulse" />
 <div className="h-48 bg-zinc-200 rounded-3xl animate-pulse" />
 </div>
 );
 ...
 const r1Rows = [
  ...
  s.username,
  s.email,
  s.isDiscontinued ? `Discontinued${s.discontinuationReason ? `: ${s.discontinuationReason}` : ""}` : "Present",
  s.activityType,
  ...
  s.username,
  s.email,
  s.isDiscontinued ? `Discontinued${s.discontinuationReason ? `: ${s.discontinuationReason}` : ""}` : "Absent",
  "-",
  "-"
  ]),
 ];
 ...
 className="w-full pl-9 pr-4 py-2 bg-zinc-50 border border-zinc-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-600 outline-none transition"
 />
 </div>
 ...
 className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-zinc-500 hover:text-zinc-900 :text-white bg-zinc-100 rounded-xl transition"
 >
 ...
 <tbody>
  ...
  <tr key={i} className="border-b ">
   <td className="p-2">
    ...
    {s.isDiscontinued && (
     <span className="ml-1 text-xs px-2 py-0.5 rounded-full bg-red-100 text-red-600 font-bold" title={s.discontinuationReason || "No reason provided"}>Discontinued{s.discontinuationReason ? `: ${s.discontinuationReason}` : ""}</span>
    )}
   </td>
  <td className="p-2">
  <span
  ...
  >
   {s.isDiscontinued ? "Discontinued" : s.status}
  </span>
  </td>
 ...
 >
  <div className="flex items-center justify-between p-3 bg-zinc-50 ">
   <div className="flex items-center gap-2">
    ...
    {s.isDiscontinued && (
     <span className="text-xs px-2 py-0.5 rounded-full bg-red-100 text-red-600 font-bold" title={s.discontinuationReason || "No reason provided"}>Discontinued{s.discontinuationReason ? `: ${s.discontinuationReason}` : ""}</span>
    )}
   </div>
   <span className="text-xs text-zinc-500">
