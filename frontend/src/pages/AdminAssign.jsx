...
const SearchableSelect = ({ options, value, onChange, multiple, placeholder, assignedValues = [] }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <div className="flex flex-wrap gap-2 flex-1 items-center">
          {multiple ? (
            ...
            <span className={value ? "text-zinc-900 dark:text-white font-medium" : "text-zinc-500"}>
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col transform opacity-100 scale-100 transition-all origin-top">
          <div className="p-3 border-b border-zinc-100 dark:border-zinc-800 flex items-center gap-3 bg-zinc-50/50 dark:bg-zinc-900/50">
            <Search className="w-4 h-4 text-zinc-400" />
            <input 
              type="text" 
              className="w-full bg-transparent border-none outline-none text-sm dark:text-white placeholder-zinc-400" 
          ...
          <div className="max-h-60 overflow-y-auto p-1.5 custom-scrollbar bg-white dark:bg-zinc-900">
              ...
              filteredOptions.map(opt => {
                ...
                return (
                  ...
                  >
                    ...
                    {isAssigned ? (
                      <span className="text-[11px] uppercase tracking-wider bg-zinc-200 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-300 px-2.5 py-1 rounded-full font-bold">
                        Already Assigned
                      </span>
                    ) : isSelected && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col h-full hover:shadow-lg transition-shadow duration-300">
          <div className="flex items-center mb-8">
            <div className="p-3.5 bg-violet-50 dark:bg-violet-900/20 rounded-2xl mr-4 shadow-sm">
              <GraduationCap className="w-6 h-6 text-violet-600 dark:text-violet-400" />
            </div>
            <div>
              <h3 className="text-xl font-bold tracking-tight text-zinc-900 dark:text-white">Assign Course to Student</h3>
          ...
          <form onSubmit={handleEnrollStudent} className="space-y-7 flex flex-col flex-1">
            <div className="space-y-2.5">
              <label className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Select Student</label>
              <SearchableSelect 
            ...
            <div className="space-y-2.5">
              <label className="text-sm font-semibold text-zinc-700 dark:text-zinc-300 flex justify-between">
                ...
                {selectedCourses.length > 0 && <span className="text-violet-600 dark:text-violet-400">{selectedCourses.length} selected</span>}
              </label>
              <SearchableSelect 
        ...
        <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col h-full hover:shadow-lg transition-shadow duration-300">
          ...
          <form onSubmit={handleAssignStudentToTeacher} className="space-y-7 flex flex-col flex-1">
            <div className="space-y-2.5">
              <label className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Select Teacher</label>
              <SearchableSelect 
